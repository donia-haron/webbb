    ...
    <div class="container">
      ...
      <div v-if="this.y==0" class="productcont3">
          ...
          <div class="productcont">
              <div class="product" v-for="user in all_users" v-bind:key="user">
                  <button class="btn" @click="remove(user)">
                      <i><font-awesome-icon :icon="['fas', 'times']" /></i>

                  </button>
                  <h3 class="productname"></h3>
                  <img class="card-img-top rounded-10"
                       
                :src="require(`../assets/${user.description}`)" 
                       alt="Card image cap" />
                  <p class="price">{{user['quantity']}}</p>
                  <p class="price">${{user['price']}}</p>

                  <button class="addtocart" @click="increase(user)">+</button>
                  <button class="addtocart" @click="decrease(user)">-</button>
              </div>
          </div>
